...
export function search(index, query, limit = 20) {
    ...
    const hitsPerToken = tokens.map(() => new Set());
    ...
    tokens.forEach((token, t) => {
        for (const key of strongKeys) {
            if (!key.startsWith(token))
                continue; // prefix match, so "prov" finds "provenance"
            const exact = key === token ? 2 : 1;
            for (const i of index.inverted[key]) {
                const rec = index.records[i];
                if (tokenize(rec.title).some((w) => w.startsWith(token)))
                    bump(i, 10 * exact, t);
                else
                    bump(i, 4 * exact, t);
            }
        ...
        for (const key of bodyKeys) {
            if (!key.startsWith(token))
                continue;
            for (const i of index.body[key])
                bump(i, key === token ? 2 : 1, t);
        }
    });
...
export function groupBySection(hits) {
    ...
    for (const hit of hits) {
        ...
        if (existing)
            ...
            groups.push({ section: hit.record.section, hits: [hit] });
    }
    return groups;
}
