...
import { resolveContentLink } from "./links.js";
...
function rehypeCodeBlocks() {
    return (tree) => {
        visit(tree, "element", (node, index, parent) => {
            if (node.tagName !== "pre" || !parent || index === undefined)
                return;
            if (parent.type === "element" && parent.tagName === "code-block")
                return;
            const code = node.children.find((c) => c.type === "element" && c.tagName === "code");
            if (!code)
                return;
...
function rehypeSiteLinks(repoPath) {
    return (tree) => {
        visit(tree, "element", (node) => {
            ...
            if (!resolved)
                return; // left as-is; check-links.mjs reports it against out/
...
function rehypeCollectHeadings(sink) {
    return (tree) => {
        visit(tree, "element", (node) => {
 ...
 */
export async function toHast(body, repoPath) {
    const headings = [];
 ...
 */
export async function renderMarkdown(body, repoPath) {
    ...
    const content = unified()
        ...
        components: {
            "graph-diagram": ({ chart }) => createElement(GraphDiagram, { chart }),
            "code-block": ({ lang, raw, children }) => createElement(CodeBlock, { lang, raw }, children),
        },
    })
        .stringify(tree);
