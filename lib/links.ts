...
import { getSource, listFiles } from "./content.ts";
 ...
 */
let slugCache: { patterns: Set<string>; starters: Set<string> } | null = null;

function knownSlugs(): { patterns: Set<string>; starters: Set<string> } {
  if (!slugCache) {
    slugCache = {
      patterns: new Set(
        listFiles("patterns", ".md")
 ...
 */
function githubUrl(repoPath: string, isDirectory: boolean): string {
...
export function resolveContentLink(
  ...
  if (resolved.endsWith(".md")) {
    ...
    if (doc) return { href: doc.route + (hash ? `#${hash}` : ""), external: false };
  }
  ...
  if (/^patterns(\/README\.md)?\/?$/.test(resolved)) return { href: "/patterns/", external: false };
  if (/^starters(\/README\.md)?\/?$/.test(resolved)) return { href: "/patterns/", external: false };
