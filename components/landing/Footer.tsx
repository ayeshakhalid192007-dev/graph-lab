...
function FooterLink({ href, children, internal = false }: { href: string; children: React.ReactNode; internal?: boolean }) {
  const className = "text-sm text-graphite hover:text-accent-primary transition-colors inline-flex items-center gap-1.5";
  ...
  const footerGroups = [
    ...
    },
    {
      ...
      links: [
        ...
        { label: "License", href: `https://github.com/${repo}/blob/main/LICENSE`, internal: false },
      ],
    },
  ];

  return (
    <footer className="border-t border-rule-strong bg-surface mt-20">
      <div className="max-w-6xl mx-auto px-6 py-16 sm:px-8 md:px-10">
        {/* Main footer content */}
          ...
          <div className="max-w-sm">
            <div className="flex items-center gap-3 mb-4">
              <svg width="28" height="28" viewBox="0 0 48 48" fill="none">
                <circle cx="24" cy="24" r="20" stroke="var(--accent-primary)" strokeWidth="1.5" opacity="0.8" />
                <circle cx="24" cy="24" r="5" fill="var(--accent-primary)" />
                <circle cx="24" cy="8" r="3" fill="var(--accent-primary)" opacity="0.7" />
                <circle cx="8" cy="24" r="3" fill="var(--accent-primary)" opacity="0.7" />
                <circle cx="40" cy="24" r="3" fill="var(--accent-primary)" opacity="0.7" />
                <circle cx="24" cy="40" r="3" fill="var(--accent-primary)" opacity="0.7" />
              </svg>
            ...
            <div>
              <p className="text-xs font-mono text-muted mb-2 uppercase tracking-wider">Install</p>
              <code className="block font-mono text-xs bg-surface-soft border border-card-border rounded-lg p-3 text-ink">
        ...
        <div className="mb-12 pb-12 border-b border-rule-strong">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-surface-soft border border-card-border rounded-lg p-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h3 className="font-mono font-semibold text-ink text-sm">Ayesha Khalid</h3>
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between text-xs text-muted">
          ...
          <p className="flex items-center gap-2">
            <span className="font-mono bg-surface-soft px-2 py-1 rounded border border-card-border">
              {commit.slice(0, 8)}
            </span>
            synced {synced}
          </p>
