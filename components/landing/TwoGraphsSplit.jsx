...
function Graph({ nodes, edges, dim }) {
    const at = (id) => nodes.find((n) => n.id === id);
    return (<g className={dim ? "dimmed" : undefined}>
      {edges.map(([from, to], i) => {
            const a = at(from);
            const b = at(to);
            return (<line key={`${from}-${to}`} className="edge" pathLength={1} x1={a.x} y1={a.y} x2={b.x} y2={b.y} stroke="var(--accent-primary)" strokeWidth={1.5} style={{ transitionDelay: `${i * 140}ms` }}/>);
        })}
      {nodes.map((n, i) => (<g key={n.id} className="node" style={{ transitionDelay: `${200 + i * 120}ms` }}>
          <circle cx={n.x} cy={n.y} r={7} fill="var(--surface)" stroke="var(--ink)" strokeWidth={1.5}/>
          <text x={n.x + 14} y={n.y + 4} fill="var(--graphite)" style={{ font: '11px var(--font-mono)' }}>
            {n.label}
          </text>
        </g>))}
    </g>);
}
export function TwoGraphsSplit({ highlightSide, }) {
    return (<ScrollAnimator className="diagram">
      <svg viewBox="0 0 560 200" className="h-auto w-full" role="img" aria-label="Two graphs side by side. On the left, a work-history graph: three attempts chained in order, recording what was tried. On the right, a fact graph: a Service node and an Owner node, each linked to a shared Receipt node, recording what turned out to be true.">
