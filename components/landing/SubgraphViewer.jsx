 ...
 */
export function SubgraphViewer({ fullGraph, subgraphNodeIds, }) {
    ...
    const positions = useMemo(() => {
        ...
        const cy = 130;
        const r = 96;
        return new Map(fullGraph.nodes.map((id, i) => {
    ...
    return (<div>
      <div className="flex flex-wrap items-center gap-3">
        <button type="button" aria-pressed={bounded} onClick={() => setBounded((b) => !b)} className={`mono border px-3 py-1 text-xs ${bounded
            ? "border-accent-primary bg-accent-primary text-paper"
            : "border-rule text-graphite hover:border-accent-primary"}`}>
      ...
      <ScrollAnimator className="diagram mt-4">
        <svg viewBox="0 0 600 260" className="h-auto w-full" role="img" aria-label={`A graph of ${fullGraph.nodes.length} nodes. Toggling to the bounded subgraph dims everything outside the ${subgraphNodeIds.length} nodes a worker is given, leaving ${hiddenCount} nodes visible but out of scope.`}>
          {fullGraph.edges.map(([from, to], i) => {
            const a = positions.get(from);
            const b = positions.get(to);
            const dim = bounded && !(inScope.has(from) && inScope.has(to));
            return (<line key={`${from}-${to}`} className={`edge${dim ? " dimmed" : ""}`} pathLength={1} x1={a.x} y1={a.y} x2={b.x} y2={b.y} stroke="var(--accent-primary)" strokeWidth={1.25} style={{ transitionDelay: `${i * 70}ms` }}/>);
        })}

          {fullGraph.nodes.map((id, i) => {
            const p = positions.get(id);
            const dim = bounded && !inScope.has(id);
            return (<g key={id} className={`node${dim ? " dimmed" : ""}`} style={{ transitionDelay: `${150 + i * 80}ms` }}>
                <circle cx={p.x} cy={p.y} r={8} fill="var(--surface)" stroke={bounded && inScope.has(id) ? "var(--accent-primary)" : "var(--ink)"} strokeWidth={1.5}/>
                <text x={p.x} y={p.y - 14} textAnchor="middle" fill="var(--graphite)" style={{ font: '11px var(--font-mono)' }}>
                  {id}
                </text>
              </g>);
        })}
